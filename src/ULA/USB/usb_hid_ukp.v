...
module ukp(
	...
	output				reg UKP_READY, 				// data frame is outputing
	output				UKP_STROBE,					// strobe for a byte within the frame
	output	reg [ 7: 0] UKP_DATA,					// output data when UKP_STROBE=1
	...
	always @(posedge USB_CLK) begin
		if(~RESET) begin 
			...
			if (sample) begin
				...
				if (~dmi && ~dpi) UKP_READY <= 0;	  // SE0: packet is finished. Mouses send length 4 reports.
			end
			if (ug==0) begin
				if(bitadr==24) UKP_READY <= 1;			// ignore first 3 bytes
				if(bitadr==88) UKP_READY <= 0;			// output next 8 bytes
			end
			if ((bitadr>11 & bitadr[2:0] == 3'b000) & (timing == 2)) UKP_DATA <= data;
			// Timing
			interval <= interval_cy ? 0 : interval + 1;
			record1 <= record;
			if (~record & record1) bank <= ~bank;
			// Connection status & WDT
			UKP_READYd <= UKP_READY;
			nakd <= nak;
			if (UKP_READY && ~UKP_READYd || inst_ready && state == S_OPCODE && inst == 4'b0010) 
	...
	assign UKP_STROBE = ~nrzon & UKP_READY & (bitadr[2:0] == 3'b100) & (timing == 2);
	reg	dpi, dmi; 
	reg	UKP_READYd;
	reg	nakd;
endmodule
