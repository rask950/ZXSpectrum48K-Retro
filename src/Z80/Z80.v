...
`include "Z80_Registers.vh"
	...
	reg				[ 7: 0]	OPCODE_REG;								// Opcode read from memory
...
always @(posedge CLK) begin
	...
	if (RESET) begin
		...
		`REG_PC						<= 0;							// Execution restarts here
	...
	STATE_MWT2L: begin												// Execute T2 again if WAIT is active
		WR							<= ~WAIT;						// WR becomes active until wait becomes inactive
		FSM_NEXT_STATE.T			<=  WAIT ? STATE_T3H
											 : STATE_T2H;
	end
	...
	if (FSM_LAST_M) begin											// Current instrution complete
	...
	PLA_PF_EXTD: begin												// M1(4) EXTD [ED]
		if (FSM_STATE == STATE_M1T4L) begin
			...
			FSM_NEXT_STATE.T		<= STATE_T1H;					// Move next to M1 WITHOUT checking for interrupts
		end
	end
	...
	PLA_PF_BITS: begin												// M1(4) BITS only [CB]
		if (FSM_STATE == STATE_M1T4L) begin
			...
			FSM_NEXT_STATE.T		<= STATE_M1T1H;					// Move next to M1 WITHOUT checking for interrupts
		end
	end
	...
	PLA_HALT: begin													// M1(4) HALT
		...
		if (FSM_STATE == STATE_M1T4L) begin
			if (INT & NMI) begin                                	// No interrupt has happened
				...
				FSM_LAST_M			<= TRUE;						// Indicate HALT instruction complete
			end
		end
	...
	PLA_LDR_HXY: begin												// M1(4) LD r,(HL/IX+n/IY+n)
		...
		STATE_M1T4L: begin
		   `REG_WZ					<= `CUR_HL;						// Base address to WZ ####### CHECK TIMING
			FSM_NEXT_STATE			<= (IX|IY)  ? STATE_MR1T1H 
												: STATE_MR2T1H;
		end
	...
	PLA_EX_SPHL: begin												// M1(4) EX (SP),HL/IX/IY
		...
		STATE_MR2T3L:
			FSM_NEXT_STATE.T		<= STATE_T4H;					// Extra cycle to do the exchange
